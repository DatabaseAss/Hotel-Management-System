...
CREATE TABLE BRANCH (
    ...
    BRANCHID AS 'CN' + CAST(ID AS VARCHAR(3)) PERSISTED UNIQUE,
...
CREATE TABLE ZONES (
	ZONE_BRANCHID INT NOT NULL,										
	ZNAME NVARCHAR(255) NOT NULL,								
	PRIMARY KEY(ZONE_BRANCHID, ZNAME),				
	FOREIGN KEY(ZONE_BRANCHID) REFERENCES BRANCH(ID) ON UPDATE CASCADE ON DELETE CASCADE
);

CREATE TABLE ROOMTYPE (
	TYPEID INT IDENTITY(1,1) NOT NULL PRIMARY KEY,								
	TYPENAME NVARCHAR(255) NOT NULL,										
	AREA FLOAT CHECK (AREA > 0) NOT NULL,									
	CAPACITY INT CHECK(CAPACITY > 0 AND CAPACITY < 11) NOT NULL,	
	DESCRIPTIONS NVARCHAR(255),										
);

CREATE TABLE BEDINFO (
	BED_TYPEID INT NOT NULL,											
	SIZE FLOAT CHECK (SIZE > 0) NOT NULL,						
	QUANTITY INT DEFAULT 1 NOT NULL CHECK(QUANTITY > 0 AND QUANTITY < 11) ,
...
CREATE TABLE BRANCH_HAVE_ROOMTYPE (
	BHR_BRANCHID INT NOT NULL,									
	BHR_TYPEID INT NOT NULL,											
	[PRICE (kVND)] INT CHECK ([PRICE (kVND)] > 0) NOT NULL,							
	PRIMARY KEY(BHR_BRANCHID, BHR_TYPEID),
	FOREIGN KEY(BHR_BRANCHID) REFERENCES BRANCH(ID)  ON DELETE CASCADE,
	FOREIGN KEY(BHR_TYPEID) REFERENCES ROOMTYPE(TYPEID)  ON DELETE CASCADE
);


-- 7. ROOM --
CREATE TABLE ROOM (
	ROOM_BRANCHID INT NOT NULL,									
	ROOMID VARCHAR(3) NOT NULL,										
	ROOM_TYPEID INT NOT NULL,												
	ROOM_ZNAME NVARCHAR(255) NOT NULL,								
...
CREATE TABLE RECEIPT (
    ID INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
    RECEIPT_BOOKINGID AS 'DP' + CONVERT(CHAR(8), CONVERT(date, BOOKING_TIME), 112) + RIGHT ('00000' + CAST(ID AS VARCHAR(6)), 6),	
    BOOKING_TIME DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CHECKIN DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CHECKOUT DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
CREATE TABLE HIRING_ROOM (
    ...
    PRIMARY KEY(HR_BRANCHID,HR_BOOKINGID,HR_ROOMID)
);

-- 18. BILL --
CREATE TABLE BILL (
    ID INT IDENTITY(1,1) NOT NULL PRIMARY KEY,
    BILLID AS 'HD' + CONVERT(CHAR(8), CONVERT(date, GETDATE()), 112) + RIGHT ('00000' + CAST(ID AS VARCHAR(6)), 6),
    -- SELECT TIME_FORMAT(%h %i),
    BILL_CHECKIN TIME NOT NULL,
    BILL_CHECKOUT TIME NOT NULL,
    BILL_BOOKINGID INT NOT NULL,
    FOREIGN KEY(BILL_BOOKINGID) REFERENCES RECEIPT(ID) ON DELETE CASCADE ON UPDATE CASCADE
);
